...
class tx_taskcenterrecent_task implements tx_taskcenter_Task {
	...
	public function getTask() {
		$content = $this->taskObject->description($GLOBALS['LANG']->getLL('link_allRecs'));
		$content .= $this->getRecentRecords();
		return $content;
	}
	...
	public function getOverview() {
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			if (is_array($pageRow)) {
				...
				$lines[] = $this->taskObject->doc->wrapClickMenuOnIcon($icon, 'pages', $pageRow['uid'], 0)  .
							$this->linkToEdit($pageRow['title'], $pageRow['uid']);
			}
		}
		...
		if (count($lines) > 0) {
			$out = '<div style="margin:3px">
						<strong>' . $GLOBALS['LANG']->sL('LLL:EXT:taskcenter_recent/locallang.xml:mod_recent', TRUE) . '</strong><br />' .
						implode('<br />', $lines) .
					'</div>';
		}
	...
	protected function getRecentRecords() {
		...
		$id = intval(t3lib_div::_GET('display'));
		...
		if (!empty($max)) {
			$this->numberOfRecentAll = ($max === 'any') ? '' : intval($max);
		}
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			if (is_array($elRow)) {
				...
				$recordTitle = $this->linkToEdit($recordTitle, $elRow['uid'], $row['tablename']);
	...
	protected function getUserSelection() {
		$out = '';
			...
			// put it all together
		$out .= '<form action="' . t3lib_div::sanitizeLocalUrl(t3lib_div::getIndPenv('TYPO3_REQUEST_URL')) . '" method="post">
...
//			20 => 'noLimit'
	 ...
	 * @return	html link
	 */
	protected function linkToEdit($linkText, $id, $table='pages') {
		$params = '&edit[' . htmlspecialchars($table) . '][' . intval($id) . ']=edit';
		$onclick = htmlspecialchars(t3lib_BEfunc::editOnClick($params, $GLOBALS['BACK_PATH']));
		$link = '<a href="#" onclick="' . $onclick . '" title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_mod_web_list.xml:edit', TRUE) . '">' . $linkText . '</a>';
